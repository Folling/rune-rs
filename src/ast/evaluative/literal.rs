            ...
            Some(Token::Textual(v)) if matches!(v.chars().next(), Some(c) if c.is_ascii_digit()) => {
                return match lexer.cur() {
                    ...
                    Some(_) => {
                        ...
                        match lexer.cur_next() {
                            ...
                            Some(Token::Textual("i8")) => Ok(Literal::Integer {
                                value: IntegerLiteral::I8(i8::from_str_radix(v, base).map_err(|e| ParseError::InvalidLiteral {
                                    got: v,
                                    expected: "-128 to 127",
                                })?),
                            }),
                            Some(Token::Textual("i16")) => Ok(Literal::Integer {
                                value: IntegerLiteral::I16(i16::from_str_radix(v, base).map_err(|e| ParseError::InvalidLiteral {
                                    got: v,
                                    expected: "-32'768 to 32'767",
                                })?),
                            }),
                            Some(Token::Textual("i32")) => Ok(Literal::Integer {
                                value: IntegerLiteral::I32(i32::from_str_radix(v, base).map_err(|e| ParseError::InvalidLiteral {
                                    got: v,
                                    expected: "-2147483648 to 2147483647",
                                })?),
                            }),
                            Some(Token::Textual("i64")) => Ok(Literal::Integer {
                                value: IntegerLiteral::I64(i64::from_str_radix(v, base).map_err(|e| ParseError::InvalidLiteral {
                                    got: v,
                                    expected: "-9223372036854775808 to 9223372036854775807",
                                })?),
                            }),
                            Some(Token::Textual("u8")) => Ok(Literal::Integer {
                                value: IntegerLiteral::U8(u8::from_str_radix(v, base).map_err(|e| ParseError::InvalidLiteral {
                                    got: v,
                                    expected: "0 to 255",
                                })?),
                            }),
                            Some(Token::Textual("u16")) => Ok(Literal::Integer {
                                value: IntegerLiteral::U16(u16::from_str_radix(v, base).map_err(|e| ParseError::InvalidLiteral {
                                    got: v,
                                    expected: "0 to 65535",
                                })?),
                            }),
                            Some(Token::Textual("u32")) => Ok(Literal::Integer {
                                value: IntegerLiteral::U32(u32::from_str_radix(v, base).map_err(|e| ParseError::InvalidLiteral {
                                    got: v,
                                    expected: "0 to 4294967295",
                                })?),
                            }),
                            Some(Token::Textual("u64")) => Ok(Literal::Integer {
                                value: IntegerLiteral::U64(u64::from_str_radix(v, base).map_err(|e| ParseError::InvalidLiteral {
                                    got: v,
                    ...
                    Some(v) => {
                        return Err(ParseError::InvalidToken {
                            got: v,
                            expected: vec![Token::Special("."), Token::Textual("any valid integer")],
                        })
                    }
                };
            }
...
impl<'a> Literal<'a> {
    fn parse_range_upper(lexer: &mut Lexer<'a>, from: Box<Expr<'a>>, lower_open: bool) -> Result<Literal<'a>, ParseError<'a>> {
        match lexer.cur() {
            None => return Err(ParseError::PrematureEOF),
            Some(Token::Special("<")) => {
                lexer.next_cur();
